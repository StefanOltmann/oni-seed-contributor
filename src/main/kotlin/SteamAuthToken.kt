    ...
    companion object {
        fun parse(raw: String): SteamAuthToken {
            ...
            require(segments.size == 3) {
                "STEAM_AUTH_TOKEN is not a JWT (expected 3 segments, got ${segments.size})"
            }
            val payloadJson = try {
                Base64.getUrlDecoder().decode(padBase64(segments[1])).decodeToString()
            } catch (e: IllegalArgumentException) {
                throw IllegalArgumentException("STEAM_AUTH_TOKEN payload is not valid base64url", e)
            }
            val claims = try {
                Json.parseToJsonElement(payloadJson) as? JsonObject
                    ?: error("STEAM_AUTH_TOKEN payload is not a JSON object")
            } catch (e: Exception) {
                throw IllegalArgumentException("STEAM_AUTH_TOKEN payload is not valid JSON: ${e.message}", e)
            }

            val steamId = claims["sub"]?.jsonPrimitive?.contentOrNull
                ?: claims["steamId"]?.jsonPrimitive?.contentOrNull
                ?: error("STEAM_AUTH_TOKEN has no 'sub' or 'steamId' claim — backend will reject it")

            val expiresAt = claims["exp"]?.jsonPrimitive?.longOrNull
            if (expiresAt != null) {
                val nowSeconds = System.currentTimeMillis() / 1000
                require(expiresAt > nowSeconds) {
                    "STEAM_AUTH_TOKEN has already expired (exp=$expiresAt, now=$nowSeconds) — refresh it from the oni-seed-browser frontend"
                }
            }
