...
/**
 * Default backend URL — overridden via the SERVER_URL env var. Matches
 * the constant the oni-seed-browser frontend uses.
 */
const val DEFAULT_SERVER_URL: String = "https://mni.stefan-oltmann.de"
...
/**
 * Posts uploads to {SERVER_URL}/upload with the same headers and body
    ...
    suspend fun upload(cluster: Cluster): UploadResult {
        ...
        val response = try {
            httpClient.post(uploadUrl) {
                ...
                header("MNI_API_KEY_BROWSER", mniApiKey)
                contentType(ContentType.Application.Json)
                setBody(payload)
            }
        } catch (e: Throwable) {
            // Connect timeouts, DNS, TLS, etc. — treat as transient.
            return UploadResult.NetworkFailure(e)
        }
...
sealed class UploadResult {
    data class Responded(val status: HttpStatusCode, val body: String?) : UploadResult()
    data class NetworkFailure(val cause: Throwable) : UploadResult()
}
