...
function decodeSnapshot(buf) {
    ...
    offset += 4;
    const worldCount = view.getUint32(offset, true);
    offset += 4;
    ...
    for (let w = 0; w < worldCount; w++) {
        ...
        for (let c = 0; c < cells; c++) {
            element_idx[c] = view.getUint16(offset, true);
            offset += 2;
            mass[c] = view.getFloat32(offset, true);
            offset += 4;
            temperature[c] = view.getFloat32(offset, true);
            offset += 4;
            disease_idx[c] = view.getUint8(offset);
            offset += 1;
            disease_count[c] = view.getInt32(offset, true);
            offset += 4;
        }
